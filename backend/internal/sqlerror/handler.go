...
func HandleError(err error) error {
	...
	if errors.As(err, &pgerr) {
		...
		switch sqlErr.Code {
		case ForeignKeyViolation:
			return errorss.NewBadRequestError(userMessage, false, &errorCode, nil, nil)
		...
		case UniqueViolation:
			...
			return errorss.NewBadRequestError(userMessage, true, &errorCode, nil, nil)

		case NotNullViolation:
			fieldErrors := []errorss.FieldError{
				{
					Field: strings.ToLower(sqlErr.ColumnName),
					Error: "is required",
				},
			}
			return errorss.NewBadRequestError(userMessage, true, &errorCode, fieldErrors, nil)
	...
	case errors.Is(err, pgx.ErrNoRows), errors.Is(err, sql.ErrNoRows):
		...
		if strings.Contains(errMsg, tablePrefix) {
			...
			return errorss.NewNotFoundError(fmt.Sprintf("%s not found",
				entityName), true, nil)
		}
		return errorss.NewNotFoundError("Resource not found", false, nil)
	}
