...
func (global *GlobalMiddlewares) GlobalErrorHandler(c *echo.Context, err error) {
	...
	case errors.As(err, &echoErr):
		status = echoErr.Code
		code = errorss.MakeUpperCaseWithUnderscores(http.StatusText(status))
		message = echoErr.Message

	default:
		status = http.StatusInternalServerError
		code = errorss.MakeUpperCaseWithUnderscores(
	...
	_ = c.JSON(status, errorss.HTTPError{
		Code:     code,
		Message:  message,
		Status:   status,
